...
// 第一步获取文件列表
fs.readdir('./img2', async function(err, files) {
    var fileList = [];
    for (var i = 0; i < files.length; i++) {
        var filename = files[i];
        var stats = fs.statSync('./img2/' + filename);
...
async function getFormHeaders(form) {
    ...
    return {
        ...
        'X-Guzzu-Access-Token': 'qhF9H5W9d70XQgkneWQkH9g75HjnY98i'
    }
}
...
async function upLoadImg(fileList) {
    ...
    for (var i = 0; i < fileList.length; i++) {
        const img = path.join(__dirname, './img2/' + fileList[i])
...
function upDateImgJson(json, filename) {
    return new Promise((resolve, reject) => {
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('imgList').insert(myObj, (err, res) => {
