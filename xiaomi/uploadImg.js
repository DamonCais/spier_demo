...
(async() => {
    ...
    imgdetails.find({}, async(err, res) => {
        ...
        for (var k in res) {
            if (res[k].type) {
                namesList.push(res[k].gid + '_' + res[k].type);
            } else {
                namesList.push(res[k].gid);
            }
        }
        fs.readdir('./imgs2', async function(err, files) {
            var fileList = [];
            for (var i = 0; i < files.length; i++) {
                var filename = files[i];
                if (namesList.indexOf(filename.split('.')[0]) === -1) {
                    var stats = fs.statSync('./imgs2/' + filename);
...
async function upLoadImg(fileList) {
    ...
    for (var i = 0; i < fileList.length; i++) {
        const img = path.join(__dirname, './imgs2/' + fileList[i])
...
function upDateImgJson(json, filename) {
    return new Promise((resolve, reject) => {
        MongoClient.connect(url, function(err, db) {
            ...
            let myObj = { gid: ids.split('_')[0], image: json.image, type: ids.split('_')[1] || '' };
